...
def process_file(file_path, option, output_folder):
    print(f"Processing file: {file_path}")
    text, data = transcribe_audio(file_path)
    
    base_name = os.path.splitext(os.path.basename(file_path))[0]
    
    if option in [0, 1]:
        transcript_file = os.path.join(output_folder, f"{base_name}_transcript.txt")
        save_transcript(text, transcript_file)
    
    if option in [0, 2]:
        subtitles = generate_subtitles(data)
        subtitles_file = os.path.join(output_folder, f"{base_name}_subtitles.txt")
        save_subtitles(subtitles, subtitles_file)
    
    if option in [0, 3]:
        summary = summarize(text)
        summary_file = os.path.join(output_folder, f"{base_name}_summary.md")
        save_summary(summary, summary_file)

def process_folder(folder_path, option):
    output_folder = os.path.join(folder_path, "output")
    os.makedirs(output_folder, exist_ok=True)
    
    for root, dirs, files in os.walk(folder_path):
        for file in files:
            if file.lower().endswith(('.mp3', '.mp4', '.wav', '.flac', '.ogg')):  # Add or remove file extensions as needed
                file_path = os.path.join(root, file)
                process_file(file_path, option, output_folder)

def main():
    if len(sys.argv) != 3:
        print("Usage: python script.py <folder_path> <option>")
        print("Options:")
        print("0: Generate all files (transcript, subtitles, summary)")
        print("1: Generate only transcript")
        print("2: Generate only subtitles")
        print("3: Generate only summary")
        sys.exit(1)
    ...
    process_folder(folder_path, option)
